...
var page=2;
var category_id=0;
Index.prototype.Listenmore=function(){
    var moremessage = $('.load-more');
...
Index.prototype.ListenActiveSwitch = function(){
    var self = this;
    var Group = $(".list-tab");
    Group.children().click(function () {
        var li = $(this);
        var category_ids = li.attr('data-category');
        page = 1;
        xfzajax.get({
            'url':'/news_list',
            'data':{
            ...
            'success':function (result) {
                if (result['code']===200){
                      var news = result['data'];
                      var tpl = template("news-item",{"newses":news});
                      var NersListGroup = $('.list-inner-group');
                      NersListGroup.empty();
                      NersListGroup.append(tpl);
                      page=2;
                      category_id=category_ids;
